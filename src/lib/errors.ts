...
function formatFatSecretApiError(error: FatSecretApiError): McpToolResponse {
	...
	switch (error.status) {
		case 400:
			...
			parts.push("  - Verify parameter formats (dates as YYYY-MM-DD, IDs as strings)");
		...
		case 404:
			parts.push("  - Verify the resource ID exists (food_id, recipe_id, etc.)");
...
/**
 * Safely extract and encode an error message for use in redirect URLs.
 * Truncates, strips non-ASCII, and URI-encodes.
 */
export function safeErrorMessage(error: unknown, fallback: string): string {
	try {
		const msg = error instanceof Error ? error.message : "Unknown error";
		return encodeURIComponent(msg.substring(0, 200).replace(/[^\x20-\x7E]/g, ""));
	} catch {
		return fallback;
	}
}
...
/**
 * Build a successful MCP tool response with a summary line and JSON data
 */
export function successResponse(summary: string, data: unknown): McpToolResponse {
	return {
		content: [
